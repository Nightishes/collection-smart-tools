...
describe('modifyHtml', () => {
  it('applies fcOverrides and fsOverrides into generated style tag and styleInfo', () => {
    const { modifiedHtml, styleInfo } = modifyHtml(sampleHtml, { fcOverrides: { fc0: '#abcdef' }, fsOverrides: { fs1: '18px' } });

    // modifiedHtml should include the overridden rules
  ...
  it('removes data images when requested', () => {
    const { modifiedHtml, imagesRemoved } = modifyHtml(sampleHtml, { removeDataImages: true });
