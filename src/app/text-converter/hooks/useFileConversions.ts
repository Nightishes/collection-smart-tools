...
export function useFileConversions() {
    ...
    try {
      ...
      if (item.isPdf) {
        if (target === 'docx') {
          ...
          const form = new FormData();
          form.append('file', item.file);
          const res = await fetch('/api/convert/pdf-to-txt', { method: 'POST', body: form });
        ...
        if (target === 'html') {
          const form = new FormData(); form.append('file', item.file); form.append('format','html');
          const res = await fetch('/api/convert/docx', { method: 'POST', body: form });
          const json = await res.json(); if (!json.success) throw new Error(json.error||'Conversion failed');
          blob = new Blob([json.content], { type: 'text/html' });
        } else if (target === 'txt') {
          const form = new FormData(); form.append('file', item.file); form.append('format','text');
          const res = await fetch('/api/convert/docx', { method: 'POST', body: form });
          const json = await res.json(); if (!json.success) throw new Error(json.error||'Conversion failed');
          blob = new Blob([json.content], { type: 'text/plain' });
        } else if (target === 'docx') {
        ...
        if (lower.endsWith('.html')) {
          if (target === 'txt') {
            const stripped = fileText.replace(/<script[^>]*>[\s\S]*?<\/script>/gi,'').replace(/<style[^>]*>[\s\S]*?<\/style>/gi,'').replace(/<[^>]+>/g,'');
            blob = new Blob([stripped], { type: 'text/plain' });
          } else if (target === 'docx') {
            const res = await fetch('/api/convert/html-to-docx', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ html: fileText, filename: filenameBase }) });
            if (!res.ok) throw new Error(await res.text());
            blob = await res.blob();
          } else if (target === 'html') {
          ...
          if (target === 'txt') {
            ...
            const escaped = fileText.replace(/&/g,'&amp;').replace(/</g,'&lt;');
      ...
      const ext = target === 'html' ? '.html' : target === 'docx' ? '.docx' : target === 'pdf' ? '.pdf' : '.txt';
      setResults(prev => [...prev, { id: item.id, target, blob, filename: filenameBase + ext }]);
    } catch (err: any) {
      setResults(prev => [...prev, { id: item.id, target, error: err?.message || 'Conversion error' }]);
    } finally {
  ...
  const clearResultsForId = (id: string) => setResults(prev => prev.filter(r => r.id !== id));
