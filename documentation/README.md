...
**For first-time setup on a new PC, see [SETUP.md](SETUP.md)** for detailed instructions.
...
| `npm run docker:build` | Build all Docker images (pdf2html, puppeteer, pull ClamAV) |
| `npm run docker:test`  | Test all Docker containers                                 |
| `npm run docker:up`    | Start docker-compose services (ClamAV)                     |
| `npm run docker:down`  | Stop docker-compose services                               |
| `npm run docker:logs`  | View docker-compose logs                                   |

See [DOCKER.md](DOCKER.md) for detailed Docker management guide.
...
This builds pdf2html, puppeteer, and pulls ClamAV images.

### Docker Container Architecture

Both images are automatically used by the API endpoints:

- `pdf2html` - PDF to HTML conversion (pdf2htmlEX)
- `collection-tools-puppeteer` - HTML to PDF conversion (Chromium/Puppeteer)

**Testing:** Run `npm run docker:test` to verify all containers work correctly (including ClamAV health check).
...
**Malware Protection**: The application can scan all uploaded files for viruses using ClamAV before processing.

**Setup**:

1. Start ClamAV daemon: `docker-compose up -d clamav`
2. Wait 5-10 minutes for virus definitions to download
3. Enable in `.env`: `VIRUS_SCAN_ENABLED=true`

**Behavior**:

- **Development** (`VIRUS_SCAN_ENABLED=false`): Files processed without scanning
- **Production** (`VIRUS_SCAN_ENABLED=true`): Files rejected if infected or ClamAV unavailable

**Scanned Routes**:

- `POST /api/upload` - PDF uploads
- `POST /api/convert/docx` - DOCX uploads

Infected files return HTTP 400 with error details. See [SETUP.md](SETUP.md) for configuration.
