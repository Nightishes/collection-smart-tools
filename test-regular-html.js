...
console.log('Original contains script tags:', maliciousHtml.includes('<script'));
console.log('Original contains iframe tags:', maliciousHtml.includes('<iframe'));
    ...
    res.on('end', () => {
        ...
        if (res.statusCode === 200) {
            ...
            if (fs.existsSync(savedPath)) {
                ...
                console.log('Saved contains iframe tags:', savedContent.includes('<iframe'));
                console.log('Saved contains onclick:', savedContent.includes('onclick'));
                console.log('Saved contains onerror:', savedContent.includes('onerror'));
                
                if (!savedContent.includes('<script') && !savedContent.includes('<iframe') && 
                    !savedContent.includes('onclick') && !savedContent.includes('onerror')) {
                    console.log('✓ Regular HTML properly sanitized - dangerous elements removed');
                } else {
